...
names_one_by_one_gen() {
  if [ -e $1 ]; then
    if ! [ -f $1 ]; then
      ...
      index=0
      while [ $count -lt $max_quantity ] && [ $index -lt $max_index ]; do
        char_count=0
        arr_temp=($var_temp)  #
        dir_name_temp=""  #
        while [ $char_count -lt $max_name ] && [ $count -lt $max_quantity ]; do
          arr_temp[$index]=$(echo ${arr_temp[$index]} | sed s/${array_src[$index]}/${array_src[$index]}${array_src[$index]}/)
  ...
  if [ -e $1 ]; then
    if ! [ -f $1 ]; then
      ...
      while [ $pairs_parameter -lt $max_index ] && [ $count -lt $max_quantity ]; do
        ...
        arr_temp=($var_temp)  #
        dir_name_temp=""  #
        while [ $index -lt $max_pairs ] && [ $count -lt $max_quantity ]; do
          let char_count=0
          while [ $char_count -lt $max_name ] && [ $count -lt $max_quantity ]; do
            arr_temp[$index]=$(echo ${arr_temp[$index]} | sed s/${array_src[$index]}/${array_src[$index]}${array_src[$index]}/)
            let index=$index+$pairs_parameter  # 2 param
            arr_temp[$index]=$(echo ${arr_temp[$index]} | sed s/${array_src[$index]}/${array_src[$index]}${array_src[$index]}/)
            let index=$index-$pairs_parameter  # 3 param
            let count=$count+1
    ...
    if ! [ -f $1 ]; then
      ...
      while [ $shift_combs -lt $iter_combinations ] && [ $count -lt $max_quantity ];do
        ...
        while [ $iter -lt $iter_combinations ] && [ $count -lt $max_quantity ];do
          ...
          while [ $index -lt $max_three_pairs ] && [ $count -lt $max_quantity ]; do
            let char_count=0
            while [ $char_count -lt $max_name ] && [ $count -lt $max_quantity ]; do
              arr_temp[$index]=$(echo ${arr_temp[$index]} | sed s/${array_src[$index]}/${array_src[$index]}${array_src[$index]}/)
              let index=$index+$shift_combs  # 2 param = $shift_combs
              arr_temp[$index]=$(echo ${arr_temp[$index]} | sed s/${array_src[$index]}/${array_src[$index]}${array_src[$index]}/)
              let index=$index+$iter+1-$shift_combs  # 3 param = $iter 
              arr_temp[$index]=$(echo ${arr_temp[$index]} | sed s/${array_src[$index]}/${array_src[$index]}${array_src[$index]}/)
              let index=$index-$iter-1  # 4 param = $iter+1
              let count=$count+1
